...
\[\p(c \vert d) = \frac{\p(d \vert c)\p(c)}{\p(d)}, \forall c \in C \]
...
Le SVM \citep{vapnik1995statlearning} est un algorithme de classification binaire, qui construit, lors de la phase d'entrainement un hyperplan séparant de manière optimale les points, représentants les exemples d'entrainement, suivant leur classe (Figure \ref{fig:sensresultat:svm}\footnote{\url{http://www.clrc.rhul.ac.uk/svm.html}}). L'hyperplan optimal est situé à égale distance des hyperplans parallèles formés par les points les plus proches des deux classes. La séparation des classes est réalisée dans un nouvel espace de haute dimension où une discrimination linéaire (plus précisément planaire) est optimale entre les classes. Ce nouvel espace résulte d'une transformation de l'espace original  (caractéristiques ou attributs des données)réalisée par une fonction \og noyau \fg{}, que nous appellerons $f$. Celle-ci, appliquée à tout vecteur $x$ en entrée, \textcolor{red}{et vecteurs de support}, peut être une simple combinaison linéaire (noyau linéaire) ou polynomial (noyau polynomial) des attributs de $x$, ou une fonction plus complexe comme le noyau Gaussien $f(x) = \exp{-\gamma \norm{x-x}}$ encore appelé.

La classification d'un nouvel objet consiste à projeter son vecteur de caractéristiques dans cet espace, et le label qui lui est prédit est celui associé à la classe du coté où il se trouve. 
...
C'est ses nombreux avantages que la PLS est appliquée avec succès pour divers problèmes de régression \citep{lacroux2011avantagesPLS}
