...
%\textcolor{red}{quelle est l'entrée? la demande? comment elle est modélisée?}
%préciser l'importance de la tâche pour le métier: analyse descriptive du sens du résultat.
...
%\textcolor{red}{commencer le postulat: qui sont des observations! en précisant que le choix de motivation est motivé par l'expertise du métier.}
...
$P(c)$ est estimée par la proportion de documents classés dans $c$ parmi les exemples d'apprentissage: $P(c) = \frac{\setsize{D_c}}{N}, \forall c \in C$.
...
\begin{hypothese}[indépendance conditionnelle des descripteurs] [Un modèle naïf bayésien étant de type génératif], étant donnée la catégorie du texte, la position de chaque mot dans le texte est générée indépendamment de tout autre mot. \label{hypo:sens_rst:iid}
\end{hypothese}
...
%	La sensibilité au nombre de voisins peut être atténuée en pondérant les points par leur distance à l'objet à classer. Il a été proposé plusieurs variantes de cette stratégie de \og vote pondéré par la distance \fg{}, comme par exemple:
%	\begin{itemize}
...
% http://eric.univ-lyon2.fr/~ricco/cours/slides/analyse_discriminante.pdf
...
L'analyse discriminante linéaire (LDA) \citep{fisher1936linearDA} %\citep{fukuaga1990StatPattRecogn-LDA, duda2012pattern-LDA} 
...
% source: http://eric.univ-lyon2.fr/~ricco/cours/slides/analyse_discriminante.pdf
% preuve à : 
% (1) https://arxiv.org/pdf/1906.02590.pdf tuto 2019 LDA vs QDA 
% (2) http://www.math.u-bordeaux.fr/~mchave100p/wordpress/wp-content/uploads/2013/10/Analyse_discrim.pdf
...
\subsubsection{fastText}
 Le classifieur fastText \citep{grave2017fasttextcls} est un réseau de neurones dont l'architecture est donnée à la \figureref{fig:sensresultat:fastText} \citep{zolotov2017analysisOffastText}. Tout document $d$ à classifier est représenté un vecteur $x$ qui est la moyenne des vecteurs des mots de $d$ \citep{zolotov2017analysisOffastText}: \[x = \frac{1}{\setsize{d}} \sum\limits_{i = 1}^{\setsize{d}} \overrightarrow{d[i]}.\]
 ...
 Ainsi, la classe de $d$ est $y = \argmax_{j \in \lbrace 1,...,\setsize{C} \rbrace} P(c_j)$. Par ailleurs, $z = B \cdot x$ où $B$ est la matrice $\setsize{C} \times n$ des poids de la couche cachée; $n$ étant le nombre d'unités de cette dernière. $B$ est apprise par l'entraînement du réseau sur un jeu de données annotées.
 
 \begin{figure}[!htb]
 	\centering
 	\includegraphics[width=0.8\textwidth]{fastText_LBoW_fr.png}
...
\section{Adaptations de la régression Gini-PLS pour la classification des textes}
...
\[classe(x) = \left\{\begin{array}{ll}
 ...
 %Cette prévision est comparée aux modèles standards SVM, LOGIT, Bayes et LDA lorsque les données tests sont projetées dans le sous-espace $\{t_1,\ldots,t_{h^*}\}$.


\begin{algorithm}[h]
	\small
	\KwData{$X$ (observations), $h_{max}$ (nombre maximum de composantes), $\nu_{max}$ (valeur maximale du paramètre $\nu$)}
	\KwResult{Composantes principales $t_1,\ldots,t_{h^*}$}
	\Repeat{ $\nu=\nu_{max}$ $[\nu=\nu+1]$ }{
		\Repeat{ $h=h_{max}$ $[h=h+1]$ }{
			$\max \cog_\nu(y,w_hX)$ s.t. $\| w_h\|=1$ $\Longrightarrow$  poids $w_h$ de $X$ \;
			MCO équation: $y=\sum_h c_ht_h +\eps_h$ \;
			MCO équation: $R(x_j)=\sum_h \beta_ht_h + \epsilon_k$ $\forall k=1,\ldots,m$ \;
			$X:=(\hat{\epsilon}_1,\ldots,\hat{\epsilon}_m)$ \;  $y:=\hat{\eps}_h$ \; }
...
\begin{algorithm}[h]
	\small\KwData{$X$ (observations), $h_{max}$ (nombre maximum de composantes), $\nu_{max}$ (valeur maximale du paramètre $\nu$)}
	\KwResult{Composantes principales $t_1,\ldots,t_{h^*}$}
	\Repeat{ $\nu=\nu_{max}$ $[\nu=\nu+1]$ }{
		\Repeat{ $h=h_{max}$ $[h=h+1]$ }{
...
%\textcolor{red}{0. expérimenter les rerésentations Doc2vec 1. sélection du $\nu$ pour le ginipls. 2. comparaisons des classifieurs en moyenne sur les représentations. 3. comparaison des représentations en moyenne sur les classifieurs. 4. comparaison des algos avec sélection de la meilleure représentation.}


Nous discutons ici les performances de divers algorithmes populaires et l'impact de la quantité et du déséquilibre des données, de l'heuristique, et de la restriction explicite des documents aux passages relatifs à la catégorie de demandes, ainsi que leur capacité à faire abstraction des autres demandes du document. {Ces expériences visent aussi à comparer l'efficacité du Gini-Logit-PLS par rapport à d'autres analyses discriminantes}. Comme \citet{im2017textclasstermweighting}, nous comparons différentes combinaisons d'algorithmes de classification et méthodes de pondération de termes (utilisées pour la représentation des textes).  Ces combinaisons représente un total 450 configurations dont:
\begin{itemize}
	\item 11 algorithmes de classification: NB, SVM, KNN, LDA, QDA, Arbre, fastText, NBSVM, Gini-PLS, Logit-PLS \citep{tenenhaus2005logitpls}, LOGIT-Gini-PLS;
	\item 10 pondérations globales de termes: (cf. \ref{sec:quanta:extract-terminologie-domaine}): $\chi^2$, $dbidf$, $\delta_{DF}$, $dsidf$, $gss$, $idf$, $ig$, $mar$, $ngl$, $rf$;
	\item 5 pondérations locales de termes (cf. \tableref{tab:sensresultat:metriq_locales}): $tf$, $tp$, $logtf$, $atf$, $logave$.
\end{itemize}
 
  La méthode Gini-Logit-PLS réalisant une projection dans un espace de petite dimension, elle a été comparée aussi à d'autres méthode de réduction de dimension.
...
% = \frac{2 \times P_{macro} \times R_{macro}}{P_{macro} + R_{macro}}$, 
...
\begin{table}[htb]
	...
	\begin{tabular}{|l|p{0.7\textwidth}|}
		...
		%fastText & valeurs par défa \\ \hline
...
\begin{table}[!htb]	
	...
	\end{tabular}
\caption{Comparaison des algorithmes proposées avec les arbres, fastText et NBSVM pour la détection du sens du résultat.}\label{tab:sensrst:global}
\end{table}
...
\caption{Évaluation de fastText et NBSVM pour l'identification du sens du résultat par classe.}\label{tab:sensrst:fasttextnbsvm}
 ...
 Les algorithmes PLS dépassent systématiquement les performances ($F_1$-mesure) de Fastext et
...
\begin{table}[!htb]
	...
	\begin{tabular}{|l|l|l|l|l|}
		...
		\multirow{3}{*}{\textit{acpa}} & \textbf{demande\_resultat\_a\_resultat\_context} & \textbf{DBIDF*TF} & \textbf{Tree} & \textbf{0.846} \\ 
		 & litige\_motifs\_dispositif & DELTADF*TF & StandardPLS & 0.697 \\ 
		 & litige\_motifs\_dispositif & AVERAGEGlobals*TF & LogitPLS & 0.683 \\ \hline
		 
		\multirow{3}{*}{\textit{concdel}} & \textbf{litige\_motifs\_dispositif} & \textbf{GSS*TF} & \textbf{Tree} & \textbf{0.798} \\ 
		 & motifs & IDF*TF & GiniLogitPLS & 0.703 \\ 
		 & context & DBIDF*LOGAVE & StandardPLS & 0.657 \\ \hline
		 
		\multirow{3}{*}{\textit{danais}} & \textbf{demande\_resultat\_a\_resultat\_context} & \textbf{CHI2*AVERAGELocals} & \textbf{Tree} & \textbf{0.813} \\ 
		...
		\multirow{3}{*}{\textit{dcppc}} & \textbf{demande\_resultat\_a\_resultat\_context} & \textbf{CHI2*TF} & \textbf{Tree} & \textbf{0.985} \\ 
		...
		\multirow{3}{*}{\textit{doris}} & \textbf{litige\_motifs\_dispositif} & \textbf{DSIDF*TP} & \textbf{GiniPLS} & \textbf{0.806} \\
		 & litige\_motifs\_dispositif & DSIDF*TP & GiniLogitPLS & 0.806 \\
		 & litige\_motifs\_dispositif & IG*ATF & StandardPLS & 0.772 \\ \hline
		 
		\multirow{3}{*}{\textit{styx}} & \textbf{motifs} & \textbf{DSIDF*TF} & \textbf{Tree} & \textbf{1} \\ 
...
Après réduction de la taille du document, force est de constater que les arbres fournissent
