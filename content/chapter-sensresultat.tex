...
\begin{figure}
\centering
\includegraphics[width=0.85\textwidth]{chartDistrSens.png}
...
Ces méthodes sont généralement basée sur une représentation  des textes dans un espace vectoriel $\mathcal{X}$ d'entrée et délimitent une frontière entre les classes dans un espace multidimensionnel.  
...
Si l'ensemble des termes de $d$ est $\lbrace t_{1}, \dots, t_{K} \rbrace$ $\subset T$ (vocabulaire), alors grâce à l'hypothèse \ref{hypo:sens_rst:iid},
...
\begin{figure}[!htb]
	\centering
	\includegraphics[width=0.5\textwidth]{Inkedsvm_LI.jpg}
...
\begin{enumerate}
	...
	\item le nombre de voisins (c'est-à-dire la valeur de $k$) qui ne doit être ni trop petit (sensibilité aux bruits / \textit{outliers}), ni trop grand (risque d'avoir dans le voisinage beaucoup de points d'une autre classe). La sensibilité au nombre de voisins peut être atténuée en pondérant les points par leur distance à l'objet à classer. Il a été proposé plusieurs variantes de cette stratégie de \og vote pondéré par la distance \fg{}, comme par exemple:
	\begin{itemize}
	...
	\item la métrique de calcul de distance qui doit être adéquate pour le type de donnée et la tâche, comme par exemple, la distance cosinus qui est préférable à la distance euclidienne pour la classification de documents, la deuxième métrique se dégradant lorsque le nombre d'attributs augmente \citep{sohangir2017improvedCosSim, aggarwal2001behaviorOfDistHighDim}.
\end{enumerate}
...
La classe de $X$ est donc $y_{k*} = \argmax_k P(Y=y_k \vert X)$, avec $P(Y=y_k \vert X) \propto P(Y=y_k) P(X \vert Y=y_k)$ car le dénominateur est le même pour toutes les classes. Dans cette expression, $P(Y=y_k)$ est la proportion d'exemples de classes $y_k$ dans l'ensemble des données d'apprentissage. Il ne reste donc qu'à déterminer $P(X \vert Y=y_k)$, pour trouver $y$. Deux hypothèses simplifient les calculs:
\begin{enumerate}
	\item l'hypothèse de normalité statuant que la probabilité conditionnelle $P(X \vert Y)$ suit une loi normale multivariée: \[P(X \vert Y = y_k) = \frac{1}{\sqrt{2\pi det(V_k)}}e^{-\frac{1}{2}(X - \mu_k)'V_k^{-1}(X - \mu_k)} \] $\mu_k$ étant le centre de gravité conditionnel, et $V_k$ la matrice de variance-covariance de la classe $y_k$;
...
L'analyse discriminante linéaire (LDA) \citep{fukuaga1990StatPattRecogn-LDA, duda2012pattern-LDA} est définie par une simplification de $P(X \vert y_k)$ sous ces deux hypothèses. En effet, grâce à la proportionnalité de la probabilité conditionnelle à :$\ln\left[P(X \vert y_k)\right] \propto -\frac{1}{2}( X - \mu_k )'V^{-1}(X - \mu_k )$\textcolor{red}{(Vérifier la position de la transposée)}, on déduit une fonction discriminante (ou de classement) linéaire proportionnelle à $P(y_k \vert X)$: \[d(y_k, X) = \ln\left[P(Y = y_k)\right] + \mu_k V^{-1}X' - \frac{1}{2}\mu_k V^{-1}\mu_k'.\] Ainsi $y_{k*} = \argmax_{k \in \lbrace 1,...,K \rbrace} d(y_k, X)$.

L'analyse discriminante quadratique (QDA) \citep{McLachlan1992DiscrAnalyStatPattRecog-QDA} considère l'hétéroscédasticité (i.e. $\exists k \neq j, V_k \neq V_j$), et donc ne s'appuie que sur la première hypothèse (normalité). Dans ce cas, on obtient une règle quadratique de classification $k* = \argmax_{k \in \lbrace 1,...,K \rbrace} Q_k(X)$ où:
