...
En effet, le document est considéré comme divisé en $N$ parties (10 dans nos expériences). La position relative d'une ligne est donc le numéro de la partie contenant la ligne. En résumé, les caractéristiques sont décrites comme suit (avec leurs étiquettes entre parenthèses):
\begin{itemize}
...
\begin{table}[!htb]
	...
	\begin{tabular}{|l|l|}
		 \hline
		Type & Descripteur \\ \hline
		forme & la ligne entière (\verb|l|)  \\ \hline
		contexte & le numéro de ligne (\verb|absNum|) \\ \hline
	\end{tabular}
\caption{Descripteurs candidats de lignes pour les sections.}
\end{table}
...
\begin{table}[!htb]
	...
	\begin{tabular}{|l|l|}
		\hline
		Type & Descripteur \\ \hline
		forme & le mot (\verb|token|)  \\ \hline
		contexte & les mots précédents (\verb|w-2, w-1|)  \\ \hline
	\end{tabular}
	\caption{Descripteurs candidats de mots pour les mentions d'entités.}
\end{table}
...
\end{figure}
 ...
 La proximité crée aussi des confusions entre les sections CORPS et DISPOSITIF qui se suivent (\figureref{fig:structuration:matrice-confusion-section}). 
