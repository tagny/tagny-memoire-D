...
\begin{figure}[!htb]
	...
	\caption{Evolution de la largeur moyenne de la silhouette pour le k-means et la distance apprise $Dis_M$}\label{fig:similarite:optimalK_with_silhouette_kmeans_dis_m_K2-30}
\end{figure}
 

\subsubsection{Comparaison avec d'autres algorithmes de catégorisation}

\subsection{Catégorisation des corpus non annotés manuellement}
Les matrices document-terme des autres catégories sont construites à base des mêmes représentations sélectionnées sur la catégorisation manuel (cf. \ref{sec:similarite:select-repr-optimal}). Le tableau \ref{tab:similarite:validation-nonsupervisee} présente les valeurs de silhouette obtenues par le k-means et le k-medoid sur ces matrices.
\begin{table}[!htb]
	\footnotesize
	\begin{center}
