...
La base d'entraînement $B$ comprend 935 documents dont 10 documents synthétiques générés pour chacun des 74 documents n'ayant qu'un seul label (circonstance factuelle). Sur un modèle TF-IDF, la régression linéaire donne un faible taux d'erreur (Figure \ref{fig:similarite:eval-regression}).

\begin{figure}[!htb]
	\centering \includegraphics[width=0.8\textwidth]{eval-lr.png} \hfil
	%\includegraphics[width=0.49\textwidth]{eval-mlp.png}
	
	\caption{Evaluation de la distance apprise: Différence entre valeurs prédites et  valeurs attendues}\label{fig:similarite:eval-regression}
\end{figure}
...
\begin{table}[!htb]
	...
	\begin{tabular}[pos]{|l|c|l|}
		\hline
		\textbf{Distance}&\textbf{Base$^a$}&\textbf{Silhouette optimale   (pondération, reduction, dim)} \\ \hline
...
Le modèle vectoriel TP-NGL réduit à 4 dimension par la factorisation de matrice non négative est préférée par la majorité des distances. L
...
Nous comparons ici l'emploie de la statistique d'écart et de la silhouette pour la détermination du nombre de cluster pour chaque distance. La sélection est effectué pour les valeurs 2 et 10. Même si $Dis_m$ retrouve le nombre attendu avec le k-means, $4$ est la plus valeur la plus choisie par les distances, et elle semble meilleure avec une F1-mesure maximale à 0.551 et un ARI de 0.398 avec $Dis_{cosine}$. Les valeurs déterminées sont assez proche de la valeur attendue 3.   
