...
Plus récemment, \citet{tibshirani2001gap_statistic} proposent la méthode de "statistique d'écart" (\textit{gap statistic}) dont le principe est de sélectionner le $K$ dont l'erreur sur le clustering de $\mathcal{D}$ est le plus éloignée possible de l'erreur moyenne obtenu sur les clusterings de $M$ ensembles $\mathcal{V}_q, 1\leq q \leq M$ de $N$ vecteurs aléatoirement générés. L'erreur sur le clustering de l'ensemble $\mathcal{A}$ est  $W_K(\mathcal{A})=\sum\limits^K_{r=1}\frac{1}{n_r}\sum\limits_{i,i' \in C_r} Dis(i,i')$, $C_r$ étant le $r^\text{ième}$ cluster de taille $n_r$ de $\mathcal{A}$. Le nombre optimal de clusters est la valeur de $K$ qui maximise $Gap_M(K) = \frac{1}{M}\sum\limits_{q=1}^M \log{W_K(\mathcal{V}_q)} - \log{W_K(\mathcal{A})}$ parmi un ensemble de candidats données.
...
\begin{itemize}
	...
	\end{table}
ARI a des valeurs dans $[-1;1]$. Une valeur négative indique que le regroupement obtenu s'accorde moins bien avec l'attendu qu'un regroupement aléatoire. 
...
\section{Méthode}
modèle vectoriel optimal > clustering par kmeans
...
Le clustering par l'algorithme des K-moyennes dépend fortement de la représentation vectorielle choisie car la facilité à reconnaître les clusters n'est qu'une conséquence de leur séparation et cohésion dans l'espace. Disposant de données annotées manuellement, nous proposons d'explorer différents modèles vectoriels, zones du documents, et méthodes réduction de dimension pour déterminer ceux qui séparent ces données aussi bien que le clustering manuel. La représentation optimale maximise la largeur moyenne de la silhouette du clustering manuel. L'idée étant d'avoir une représentation qui optimise à la fois les métriques supervisées et non supervisées de validation. Le modèle vectoriel ainsi déterminé est utilisé ensuite pour le clustering de corpus non annoté. Le Tableau \ref{tab:similarite:silhouette-vecteur-manuel} présente le modèle vectoriel de largeur moyenne de silhouette optimal sur les données annotées pour chaque distance.

\begin{table}[!htb]
	\small
	\begin{center}
	\begin{tabular}[pos]{|l|c|c|l|}
		\hline
		\textbf{Distance}&\textbf{Base$^a$}&\textbf{Silhouette} & \textbf{(pondération, reduction, dim)} \\ \hline
		$Dis_{jaccard}$ & 0.001 & 0.212 &(TP-NGL, NMF, 4) \\ \hline
		$Dis_{cos}$ & 0.002 & 0.202 & (TP-NGL, NMF, 4) \\ \hline
		$Dis_{M}$ & -0.049 & 0.195 &(TP-NGL, NMF, 4) \\ \hline
		$Dis_{braycurtis}$ & 0.002& 0.182 &(TP-NGL, NMF, 4) \\ \hline
		$Dis_{euclidienne}$ & 0.001& 0.168 & (TP-NGL, NMF, 4) \\ \hline
		$Dis_{manhattan}$ & -0.019& 0.17  & (TP-NGL, NMF, 4) \\ \hline
...
\end{center}
    ...
    \caption{Meilleure représentation vectorielle pour chaque distance sur le clustering manuel.} \label{tab:similarite:silhouette-vecteur-manuel}
\end{table}

\subsection{Clustering du corpus annotées manuellement}
\subsubsection{Nombre de clusters prédéfini}
La représentation vectorielle qui donne la meilleure silhouette pour le clustering manuel est celle qui permet une meilleure séparation entre les clusters et cohésion dans chaque cluster, donc par conséquent une meilleur distribution des points dans l'espace pour reconnaître des cluster donc la sémantique est de même nature que celle de l'annotation manuelle. Pour chaque distance, le modèle vectoriel optimal sur le clustering manuel devrait être optimale pour un clustering selon les métriques d'évaluation supervisée. Le Tableau \ref{tab:similarite:validation-supervisee} présente les mesures ARI, NMI et F1-mesure du clustering par k-means et k-medoids sur $\mathcal{D}_{arcpa}$, avec $K=3$.
...
\begin{table}[!htb]
	...
	\begin{tabular}[pos]{|l|l|c|c|c|c|c|c|}
		...
		\textbf{Distance}& \textbf{Algorithme}& \textbf{Silhouette}& \textbf{ARI} & \textbf{NMI} & \textbf{R} & \textbf{P} & \textbf{F1} \\ \hline
		 ...
		 dis\_cosine     & kmeans    & \textbf{0.422}      & 0.389 & 0.406 & 0.556  & 0.616     & 0.583 \\ \hline
		 dis\_cosine     & kmedoids  & \textbf{0.448}      & 0.437 & 0.455 & 0.656  & 0.598     & \textbf{0.626} \\ \hline
		 ...
		 dis\_jaccard    & kmeans    & 0.442      & 0.371 & 0.389 & 0.554  & 0.600     & 0.574 \\ \hline
	...
	\end{tabular}
	\caption{Validation supervisée sur $\mathcal{D}_{arcpa}$.} \label{tab:similarite:validation-supervisee}
\end{table}

\subsubsection{Nombre de clusters déterminé automatiquement}
Nous comparons ici l'emploie de la statistique d'écart et de la silhouette pour la détermination du nombre de cluster pour chaque distance.

\subsection{Clustering des corpus sans annotation manuelle}
Les matrices document-terme des autres catégories sont construites à base des mêmes représentations sélectionnées sur le clustering manuel (cf. \ref{sec:similarite:select-repr-optimal}). Le tableau \ref{tab:similarite:validation-nonsupervisee} présente les valeurs de silhouette obtenues par le kmeans et le kmedoid sur ces matrices.
\begin{table}[!htb]
	\centering
	\begin{subfigure}[ht]{0.45\textwidth}
		\centering
	\begin{tabular}[pos]{|l|l|c|}
		\hline
		\textbf{Corpus}& \textbf{Distance}& \textbf{S}  \\ \hline
	\multirow{7}{*}{$\mathcal{D}_{acpa}$ (21)} & $Dis_{braycurtis}$&  \\ \cline{2-3}
		 & $Dis_{cos}$  &   \\ \cline{2-3}
		 & $Dis_{euclidienne}$&   \\ \cline{2-3}
		 & $Dis_{jaccard}$ &   \\ \cline{2-3}
		 & $Dis_{manhattan}$&   \\ \cline{2-3}
		 & $Dis_{pearson}$ &   \\ \cline{2-3}
		 & $Dis_{M}$ & \\ \cline{2-3}
		 & $Dis_{wmd}$ & \\ \hline
	\multirow{7}{*}{$\mathcal{D}_{concdel}$ (30)} & $Dis_{braycurtis}$&  \\ \cline{2-3}
	& $Dis_{cos}$  &   \\ \cline{2-3}
	& $Dis_{euclidienne}$&   \\ \cline{2-3}
	& $Dis_{jaccard}$ &   \\ \cline{2-3}
	& $Dis_{manhattan}$&   \\ \cline{2-3}
	...
	& $Dis_{M}$ & \\ \cline{2-3}
	& $Dis_{wmd}$ & \\ \hline
	\multirow{7}{*}{$\mathcal{D}_{danais}$ (198)} & $Dis_{braycurtis}$&  \\ \cline{2-3}
	& $Dis_{cos}$  &   \\ \cline{2-3}
	& $Dis_{euclidienne}$&   \\ \cline{2-3}
	& $Dis_{jaccard}$ &   \\ \cline{2-3}
	& $Dis_{manhattan}$&   \\ \cline{2-3}
...
\begin{subfigure}[ht]{0.45\textwidth}
	...
	\begin{tabular}[pos]{|l|l|c|}
		\hline
		\textbf{Corpus}& \textbf{Distance}& \textbf{S}  \\ \hline
		\multirow{7}{*}{$\mathcal{D}_{dcppc}$ (91)} & $Dis_{braycurtis}$&  \\ \cline{2-3}
		& $Dis_{cos}$  &   \\ \cline{2-3}
		& $Dis_{euclidienne}$&   \\ \cline{2-3}
		& $Dis_{jaccard}$ &   \\ \cline{2-3}
		& $Dis_{manhattan}$&   \\ \cline{2-3}
		...
		& $Dis_{M}$ & \\ \cline{2-3}
		& $Dis_{wmd}$ & \\ \hline
		\multirow{7}{*}{$\mathcal{D}_{doris}$ (59)} & $Dis_{braycurtis}$&  \\ \cline{2-3}
		& $Dis_{cos}$  &   \\ \cline{2-3}
		& $Dis_{euclidienne}$&   \\ \cline{2-3}
		& $Dis_{jaccard}$ &   \\ \cline{2-3}
		& $Dis_{manhattan}$&   \\ \cline{2-3}
		...
		& $Dis_{M}$ & \\ \cline{2-3}
		& $Dis_{wmd}$ & \\ \hline
		\multirow{7}{*}{$\mathcal{D}_{styx}$ (50)} & $Dis_{braycurtis}$&  \\ \cline{2-3}
		& $Dis_{cos}$  &   \\ \cline{2-3}
		& $Dis_{euclidienne}$&   \\ \cline{2-3}
		& $Dis_{jaccard}$ &   \\ \cline{2-3}
		& $Dis_{manhattan}$&   \\ \cline{2-3}
	...
	\end{tabular}
\end{subfigure} 
	\caption{Validation non-supervisée sur $\mathcal{D}_{acpa}, \mathcal{D}_{concdel}, \mathcal{D}_{danais}, \mathcal{D}_{dcppc}, \mathcal{D}_{doris}, \mathcal{D}_{styx}$.} \label{tab:similarite:validation-nonsupervisee}
\end{table}

\subsection{Terminologie des clusters}
