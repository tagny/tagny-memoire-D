 0000000\chapter{Extraction des Circonstances Factuelles}
...
\subsection{Clustering de documents}
...
Une simple implémentation de $d$ peut être de considérer le taux de mots modifiés dans $x$: 
\begin{equation}
d(x,y) = \frac{\vert\vert P_{x,y} \vert\vert}{\vert\vert W_x \vert\vert}
\end{equation}
...
\begin{equation}
d(x,y) = \sum\limits_{(w_x, w_y) \in Px,y} d_w(w_x,w_y),
\end{equation}
...
\begin{algorithm}[H]
 ...
 \Return $y$\;
 \caption{Génère une perturbation de $x$} \label{algo:perturbation}
\end{algorithm}
...
\begin{algorithm}[H]
 ...
 \caption{modifie} \label{algo:modifiemot}
\end{algorithm}

Après avoir généré la base de données $B = \lbrace (d_i, d_i', s(d_i, d_i'))\rbrace$, on entraine un mdoèle regressif pour prédire la similarité entre 2 documents en fonction de leur représentation vectorielle. Ce modèle regressif peut être utilisé comme métrique de similarité dans un algorithme de clustering comme l'algorithme des K-moyennes.

\textcolor{red}{Issues:}
\begin{itemize}
\item les docs sont généralement de tailles différentes, ne faudrait il pas intégrer une perturbation ajout de mots?
\item il faudrait intégrer la composante taille du document
\item comment assurer les propriétés d'une fonction similarité? par exemple si aucune perturbation n'est opérée, alors la similarité est maximale et si tous les mots sont modifiés alors la similarité est minimale
\item $s = 1-p$ ne fonctionnerait pas comme similarité parce que le p est la proba de modifier un mot (ce qui n'assure pas qu'on l'ait modifié). Du coup, une même perturbation pourrait avoir deux mesures de similarité différentes avec le texte de départ. Pire encore, si aucune perturbation n'a eu lieu, la similarité avec l'original ne sera pas maximale. Je propose d'utiliser le taux de perturbation à la place de $p$ dans le calcul de $s$. Ce taux correspond à la proportion de mots modifiés.
\item Ne faudrait il pas prendre en compte un poids pour les mots, car peut-être la modification de certains mots ne devrait pas avoir le même impact sur la similarité ou le taux de perturbation que celle d'autres mots. 
\item ne faudrait il pas intégré une autre similarité plus proche de notre objectif qu'on déduirait des données labelisées (appartenance ou pas au même cluster)
\end{itemize}
...
Pour l'évaluation non supervisée, les 6 catégories de demande utilisée pour l'extraction de demandes sont utilisés.
