...
\end{equation}
$d_i$ est un document du corpus original $\mathcal{D}$, et $d'_i$ est le document synthétique obtenu par modifications contrôlées de $d_i$. $\vec{d_{ik}}$ désigne le plongement lexical du mot $d_{ik}$. Pour garantir la symétrie et la réflexivité de la métrique, nous imposons respectivement ${Dis_\mathcal{M}}(d_i, d'_i) = {Dis_\mathcal{M}}(d'_i, d_i)$ et ${Dis_\mathcal{M}}(d_i, d_i) = {Dis_\mathcal{M}}(d'_i, d'_i) = 0, \forall d \in \mathcal{D}$ sur le jeu d'entraînement généré. 

L'algorithme de génération de document synthétique utilise une valeur seuil de probabilité $p$ contrôlant le taux de modifications à effectuer sur le document original (Algorithme \ref{algo:similarite:modifierdoc}). 

\begin{algorithm}[!htb] % Version française avec : https://pierre.chachatelier.fr/latex/index.php
 \KwData{document $d_i$, valeur seuil de probabilité $p$, le vocabulaire $W$}
...
Plusieurs documents sont ainsi générés pour chaque $d_i \in \mathcal{D}$ pour former une base d'entraînement $B_\mathcal{M} = \lbrace (d_i, d'_i, {Dis}(d_i, d'_i))\rbrace_{1 \leq i \leq \norm{B_\mathcal{M}}}$.
...
La métrique utilisée entre les vecteur mots est la distance cosinus.
