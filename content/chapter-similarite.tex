...
Pour analyser la validité et l'adéquation de la métrique apprise, nous l'entraînons sur la base générée puis nous l'évaluons sur le corpus annoté $\mathcal{D}_{arcpa}$ restreint aux 74 documents n'appartenant qu'à l'un des cas $L = \lbrace a, b, c \rbrace$. Quant à l'efficacité de la métrique apprise, nous l'entraînons sur toutes les données annotées générées. Les documents sont pré-traités avant leur représentation sous forme vectorielle. Ce prétraitement consiste à sectionner les documents (chapitre \ref{chap:structuration}), à n'utiliser que le contenu principal des documents (Litige + Motifs + Dispositif), à mettre en minuscule et lemmatiser ce contenu, puis à éliminer la ponctuation et des mots inutiles (\textit{stop words})  car ils sont généralement indépendants de toute catégorie. Après pré-traitement, les documents ont une taille allant de 208 à 3812 mots dont une moyenne de 1381 mots. Le vocabulaire $W$ utilisé est restreint au mots du corpus original $D$ sur lequel il faut appliquer les regroupements. La représentation vectorielle emploie les modèles de types TF-IDF avec des n-grammes de 1 à 3 mots (pour prendre en compte l'ordre entre les mots). Chaque poids global $g(t)$ d'un terme $t$ est multiplié par le logarithme du nombre de mots de $t$ donnant ainsi une nouvelle pondération globale $\hat{g}(t) = \log_2{\vert t \vert} \times g(t)$. Les poids globaux sont appris sur la discrimination entre deux corpus $\mathcal{D}_{arcpa}$ (84 documents) et $\mathcal{D}_{\overline{arcpa}}$ (427) i.e. la terminologie de $\mathcal{D}_{arcpa}$.
 ...
 La librairie Python Scikit-Learn \citep{Pedregosa2011scikit-learn} a été utilisée pour les implémentations des algorithmes de réduction de dimension. La distance WMD est celle implémentée dans la librairie Gensim de \citet{rehurek2010gensim}. Le modèle de vecteurs de mots est un modèle GLoVE \citet{pennington2014glove} entraîné sur un corpus de +800k décisions de justice lémmatisées, à l'aide de vecteurs de dimension 300 obtenu sur une taille de fenêtre de contexte égale à 15 mots.

\subsection{Validité de la distance apprise}
La base d'entraînement $B$ comprend 935 documents dont 10 documents synthétiques générés pour chacun des 74 documents n'ayant qu'un seul label (circonstance factuelle). Sur un modèle TF-IDF, la régression par un réseau de neurone (200 neurones par couches, 2 couches internes) donne un taux d'erreur inférieur à celui d'une régression linéaire (Figure \ref{fig:similarite:eval-regression}) c'est pourquoi nous avons choisi la régression par réseau de neurone.
...
\begin{table}[!htb]
	...
	\begin{tabular}[pos]{|l|c|c|l|}
		\hline
		\textbf{Distance}&\textbf{Base$^a$}&\textbf{Silhouette} & \textbf{(lw-gw, reduction, dim, n-gram)} \\ \hline
		$Dis_{jaccard}$ & 0.002 & 0.217 &(TP-NGL, NMF, 4, 1-4) \\ \hline
		$Dis_{cos}$ & 0.003& 0.205 & (LOGAVE-NGL, NMF, 5, 1-6) \\ \hline
		$Dis_{M}$ & & 0.203 &(LOGAVE-NGL, NMF, 5, 1-6) \\ \hline
		$Dis_{braycurtis}$ & 0.002& 0.191 &(LOGAVE-NGL, NMF, 5, 1-6) \\ \hline
		$Dis_{euclidienne}$ & 0.002& 0.175 & (TP-NGL, NMF, 4, 1-4) \\ \hline
		$Dis_{manhattan}$ & -0.01& 0.173  & (TP-NGL, NMF, 4, 1-4) \\ \hline
		$Dis_{pearson}$ & 0.009&0.112 &(TP-IG, NMF, 10, 1-6) \\ \hline
		$Dis_{wmd}$ & -0.096 & - & - \\ \hline
	\end{tabular}		
\end{center}
	
	$^a$ occurrence de mots pour $Diw_{wmd}$, et TF-IDF pour les autres 

    \caption{Comparaison des modèles vectoriels sur le clustering manuel.} \label{tab:similarite:silhouette-vecteur-manuel}
\end{table}

\subsection{Clustering du corpus annotées manuellement}
La représentation vectorielle qui donne la meilleure silhouette pour le clustering manuel est celle qui permet une meilleure séparation entre les clusters et cohésion dans chaque cluster, donc par conséquent une meilleur distribution des points dans l'espace pour reconnaître des cluster donc la sémantique est de même nature que celle de l'annotation manuelle. Pour chaque distance, le modèle vectoriel optimal sur le clustering manuel devrait être optimale pour un clustering selon les métriques d'évaluation supervisée. Le Tableau \ref{tab:similarite:validation-supervisee} présente les mesures ARI, NMI et F1-mesure du clustering par k-means et k-medoids sur $\mathcal{D}_{arcpa}$.

\begin{table}[!htb]
	\centering
	\begin{tabular}[pos]{|l|c|c|c|c|c|}
		\hline
		\textbf{Distance}& \textbf{ARI} & \textbf{NMI} & \textbf{Rappel} & \textbf{Précision} & \textbf{F1-mesure} \\ \hline
		 $Dis_{braycurtis}$& & & & & \\ \hline
		$Dis_{cos}$  &  & & & &  \\ \hline
