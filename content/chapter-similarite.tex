...
La métrique peut être normalisée ($\forall a,b \in \mathcal{X};  0 \leq Dis(a,b) \leq 1$). Dans ce cas, la relation entre la similarité $Sim$ et la dis-similarité $Dis$ est définie par $Sim(a,b) = 1 - Dis(a,b)$.
...
\begin{itemize}
	...
	\item la similarité cosinus: $Sim_{cos}(x,x') = \sqrt{\frac{x^Tx'}{\norm{x}\norm{x'}}}$, avec $-1 \leq Sim_{cos}(x,x') \leq +1 $
	...
	\item Le coefficient de correlation de Pearson: pour $TF_x = \sum\limits^m_{i=1} w_{x,i}, \forall x \in \mathcal{X}$,
	
	$Sim(x,x') = \frac{m \sum\limits^m_{i=1} w_{x,i} w_{x',i} - TF_xTF_{x'}}{\sqrt{[m \sum\limits^m_{i=1} w^2_{x,i} - TF^2_x][m \sum\limits^m_{i=1} w^2_{x',i} - TF^2_{x'}}}$;
	
%$Dis = 1 - Sim si Sim \geq 0; = \vert Sim \vert \text{ si }Sim < 0$
\[
Dis =
\left\{ \begin{array}{ll}
...
Suppression des \textit{stop words} car ils sont généralement indépendants de toute catégorie. lemmatisation?

La distance entre les mots est la distance cosinus.
