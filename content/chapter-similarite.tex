...
La métrique peut être normalisée ($\forall a,b \in \mathcal{X};  0 \leq Dis(a,b) \leq 1$), à l'instar des métriques cosinus et de Jaccard. Dans ce cas, la relation entre la similarité $Sim$ et la dis-similarité $Dis$ est définie par $Sim(a,b) = 1 - Dis(a,b)$.
...
\begin{itemize}
	...
	\item la similarité cosinus est basée sur la mesure de l'angle entre les vecteurs représentant les documents $x$ et $x'$, et est calculée par: \[Sim_{cos}(x,x') = \sqrt{\frac{x^Tx'}{\norm{x}\norm{x'}}}\]
...
\begin{itemize}
	\item l'information mutuelle normalisée (NMI) \citep{strehl2000nmi} normalise l'information mutuelle entre les labels attendus $Y$ et ceux des clusters $C$ par la somme de leur entropie respective: \[NMI(Y,C) = \frac{2 \cdot I(Y;C)}{H(Y) + H(C)}\]
	...
	\item la précision du clustering (ACC) \citep{}.  
\end{itemize}
...
\section{Apprentissage d'une métrique fondée la modification du document}
...
Les documents sont pré-traités avant leur représentation sous forme vectorielle. Ce prétraitement consiste en une lemmatisation, puis une élimination des caractères de ponctuation et des mots inutiles (\textit{stop words})  car ils sont généralement indépendants de toute catégorie.
